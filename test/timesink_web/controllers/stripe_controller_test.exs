defmodule TimesinkWeb.StripeControllerTest do
  ...
  describe "POST /api/webhooks/stripe.com" do
    test "handles payment_intent.succeeded and creates a FilmSubmission with a current user", %{
      conn: conn
    } do
      user = insert(:user)
      ...
      assert fs = Timesink.Repo.get_by(FilmSubmission, payment_id: "pi_test_123")
      assert fs.title == "Test Film"
      assert fs.submitted_by_id == user.id
    end

    test "handles payment_intent.succeeded and creates a FilmSubmission with no current user", %{
      conn: conn
    } do
      user = insert(:user)
